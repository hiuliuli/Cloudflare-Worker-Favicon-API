...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
  <path fill="#000" d="M415.9 344L225 344C227.9 408.5 242.2 467.9 262.5 511.4C273.9 535.9 286.2 553.2 297.6 563.8C308.8 574.3 316.5 576 320.5 576C324.5 576 332.2 574.3 343.4 563.8C354.8 553.2 367.1 535.8 378.5 511.4C398.8 467.9 413.1 408.5 416 344zM224.9 296L415.8 296C413 231.5 398.7 172.1 378.4 128.6C367 104.2 354.7 86.8 343.3 76.2C332.1 65.7 324.4 64 320.4 64C316.4 64 308.7 65.7 297.5 76.2C286.1 86.8 273.8 104.2 262.4 128.6C242.1 172.1 227.8 231.5 224.9 296zM176.9 296C180.4 210.4 202.5 130.9 234.8 78.7C142.7 111.3 74.9 195.2 65.5 296L176.9 296zM65.5 344C74.9 444.8 142.7 528.7 234.8 561.3C202.5 509.1 180.4 429.6 176.9 344L65.5 344zM463.9 344C460.4 429.6 438.3 509.1 406 561.3C498.1 528.6 565.9 444.8 575.3 344L463.9 344zM575.3 296C565.9 195.2 498.1 111.3 406 78.7C438.3 130.9 460.4 210.4 463.9 296L575.3 296z"/>
</svg>
`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (env.TOKEN) {
      if (!userToken || userToken !== env.TOKEN) {
        return new Response("Error: Invalid or missing token.", { status: 403 });
      }
    }
    ...
    try {
      const targetResp = await fetchWithTimeout(targetUrl.href, { headers }, 5000);
      ...
      if (contentType && contentType.includes("image")) {
        finalIconUrl = targetUrl.href;
         ...
         if (!targetResp.ok) throw new Error("Page fetch failed");
      ...
      const iconResp = await fetchWithTimeout(finalIconUrl, { headers }, 5000);
      
      if (!iconResp.ok) throw new Error("Icon fetch failed");
      
      const iconType = iconResp.headers.get("content-type") || "image/png";
      const iconBuffer = await iconResp.arrayBuffer();

      let finalBody;
      let finalContentType;

      if (targetSize) {
        const base64Img = arrayBufferToBase64(iconBuffer);
        finalBody = `
<svg xmlns="http://www.w3.org/2000/svg" width="${targetSize}" height="${targetSize}" viewBox="0 0 ${targetSize} ${targetSize}">
  <image href="data:${iconType};base64,${base64Img}" width="${targetSize}" height="${targetSize}" style="image-rendering: auto;" />
</svg>`;
        finalContentType = "image/svg+xml";
      } else {
        finalBody = iconBuffer;
        finalContentType = iconType;
      }
